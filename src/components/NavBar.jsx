...
const CustomLink = ({ href, title, className = "" }) => {
  ...
  return (
    <Link href={href} className={`${className} relative group`}>
      {title}
      <span
        className={`h-[1px] inline-block bg-dark absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 text-dark ${
...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
  ...
  return (
    ...
    >
      {title}
      <span
        className={`h-[1px] inline-block bg-light absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 text-slate-900 ${
          router.asPath === href ? "w-full" : "w-0"
        } dark:bg-dark`}
      >
        &nbsp;
...
const NavBar = () => {
  ...
  return (
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between sticky top-0 z-50 bg-amber-50 dark:bg-dark dark:text-light lg:relative">
      <buttton
      ...
      {isOpen ? (
        <div className="min-w-[70wv] flex flex-col justify-between z-30 items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32 px-11 transition-all duration-300 ease-in-out">
      ...
      <div className="absolute left-[50%] top-x-[-50%] lg:mt-5">
        <Logo />
      </div>
    </header>
  )
